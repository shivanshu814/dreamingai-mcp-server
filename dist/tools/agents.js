...
export function registerAgentTools(server) {
    // ── List Agents ─────────────────────────────────────────────────────
    server.tool("list_agents", "List all available AI agents in DreamingAI", {
    ...
    // ── Get Agent ───────────────────────────────────────────────────────
    server.tool("get_agent", "Get details of a specific AI agent in DreamingAI", {
    ...
    // ── Create Agent ────────────────────────────────────────────────────
    server.tool("create_agent", "Create a new AI agent in DreamingAI with custom role and behavior", {
        ...
        capabilities: z
            .array(z.string())
            .optional()
            .describe("List of capabilities to enable (e.g., 'web_search', 'code_execution')"),
    }, async ({ name, description, systemPrompt, modelId, temperature, maxTokens, capabilities, }) => {
        try {
            const body = { name };
            if (description)
    ...
    // ── Update Agent ────────────────────────────────────────────────────
    server.tool("update_agent", "Update an existing AI agent's configuration in DreamingAI", {
    ...
    }, async ({ agentId, name, description, systemPrompt, modelId, temperature, maxTokens, capabilities, }) => {
        try {
            const body = {};
            if (name)
                body.name = name;
    ...
    // ── Delete Agent ────────────────────────────────────────────────────
    server.tool("delete_agent", "Delete an AI agent from DreamingAI", {
